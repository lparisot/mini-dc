...
class BandwidthMonitor(app_manager.RyuApp):
    ...
    def statsReplyHandler(self, ev):
        ...
        if name in self.topo.edgeSwitches.keys():
            for stat in body:
                portno = stat.port_no
                if portno in self.topo.ports[name].keys():
                    if self.topo.ports[name][portno] in self.topo.hosts.keys():
                        self.bwstats.addHostBwStat(self.topo.ports[name][portno], stat.tx_bytes, stat.rx_bytes)

...
class AdaptivePolicy(object):
    ...
    def minUtilization(self):
        ...
        sortedUtilization = sorted([(self.utilization[switch], switch) for switch in self.utilization.keys()])
        return sortedUtilization[0][1]
...
class StaticPolicy(object):
    ...
    def build(self, topo):
        ...
        for edge in topo.edgeSwitches.values():
            ...
            for h in topo.hosts.values():
                if h.name in edge.neighbors:
                    # don't send it up to a core switch
                    dst = h.name
                else:
                    # use the first VLAN
                    vlanid = h.vlans[0]
                    dst = topo.getVlanCore(vlanid)
                outport = topo.ports[edge.name][dst]
                routingTable[edge.dpid].append({
                    'eth_dst' : h.eth,
                    'output' : [outport],
                    'priority' : 2,
